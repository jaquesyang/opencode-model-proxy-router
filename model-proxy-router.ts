...
export function createRouter(configPath: string) {
  ...
  // Cache of parsed route config: { key: { agentName, proxyUrl } }
  const routeAgents = new Map<string, { name: string; url: string }>()

  const makeAgent = (url: string): Any | null => {
  ...
  const reload = () => {
    ...
    for (const [name, url] of Object.entries(raw.proxies ?? {})) {
      const agent = makeAgent(url as string)
  ...
  const wrappedFetch = async (input: Any, init: Any) => {
    ...
    if (DEBUG && typeof model === "string") {
      console.warn(
        `[model-proxy-router] url=${url} model=${model} -> ${agent ? `proxy(${agent.url})` : "direct"}`
      )
    }
